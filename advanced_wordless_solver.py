...
def filter_words(word_list, length=5):
    return [word for word in word_list if not_removed(word) and len(word) == length]
...
def found_letters(w):
    letters = ""
    return all(letter in w for letter in letters)
